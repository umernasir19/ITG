...
Public Class ItemInvetoryReportForProcess
        ...
        If DtCheck.Rows(0)("Department") = "Fabric Store" Then
            ...
            Else
                ...
                If lblID.Text = 0 Then
                    ...
                    objTempStockInventory.InsertTempProcessRecv(cmbGodown.SelectedValue)
                    If cmbGodown.SelectedItem.Text = "DAL-2 MAIN F.STORE" Then
                        objTempStockInventory.InsertTempProcessIssue()
                    Else
                        objTempStockInventory.InsertTempProcessIssueAllCase(cmbGodown.SelectedValue)
                    End If
                    objTempStockInventory.InsertTempProcessGodownRecv(cmbGodown.SelectedValue)
                    objTempStockInventory.InsertTempProcessGodpwnIssue(cmbGodown.SelectedValue)
                    Dim x As Integer
                    ...
                    If ckhWithoutZeroQty.Checked = True Then
                        Report.Load(Server.MapPath("~/Reports/ItemInventoryReportForAverageAllCaseForZeroProcess.rpt"))
                    Else
                        Report.Load(Server.MapPath("~/Reports/ItemInventoryReportForAverageAllCaseProcess.rpt"))
                    End If
                    ...
                    Dim ReportName As String = "PROCESS INVENTORY"
                ...
                Else
                    ...
                    Dim lOCATIONid As Long = cmbGodown.SelectedValue
                    objTempStockInventory.TruncateTempInventoryNewForFinalInventory()
                    objTempStockInventory.InsertTempProcessRecvAnyCase(lblID.Text, cmbGodown.SelectedValue)
                    If cmbGodown.SelectedItem.Text = "DAL-2 MAIN F.STORE" Then
                        objTempStockInventory.InsertTemp11(lblID.Text)
                    Else
                        objTempStockInventory.InsertTemp2222(lblID.Text, cmbGodown.SelectedValue)
                    End If
                    objTempStockInventory.InsertTemp3333(lblID.Text, cmbGodown.SelectedValue)
                    objTempStockInventory.InsertTemp44444(lblID.Text, cmbGodown.SelectedValue)
                    Dim x As Integer
                    Dim OBJDATE As New GeneralCode
                    Dim Date1 As Date = "01/03/2015"
                    Dim SDate As String = Date1.ToString("dd/MM/yyyy")
                    ...
                    Dim FileName As String
                    If ckhWithoutZeroQty.Checked = True Then
                        Report.Load(Server.MapPath("~/Reports/ItemInventoryReportForAverageAnyCaseForZeroProcess.rpt"))
                    Else
                        Report.Load(Server.MapPath("~/Reports/ItemInventoryReportForAverageAnyCaseProcess.rpt"))
                    End If
                    FileName = "Item_Inventory_Report"
                    Report.Refresh()
                    Dim di As DirectoryInfo = New DirectoryInfo(Server.MapPath("~/TempPDF"))
    ...
    Protected Sub txtFabricCode_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtFabricCode.TextChanged
        Try
            Dim dt As DataTable
            dt = objPOMaster.GetItemFabricNewItemCategoryForProcess(txtFabricCode.Text)

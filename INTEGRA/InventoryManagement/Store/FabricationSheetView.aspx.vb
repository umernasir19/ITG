...
Public Class FabricationSheetView
    ...
    Function LoadData() As ICollection
        ...
        objDataTable = ObjFabricationSheetmst.GetView
    ...
    Protected Sub cmdAdd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdAdd.Click
        Response.Redirect("FabricationSheetAdd.aspx")
    End Sub
    ...
    Protected Sub dgView_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgView.ItemCommand
        Try
            Select Case e.CommandName
                Case "Edit"
                    Dim FabricationSheetMstID As Long = dgView.Items(e.Item.ItemIndex).Cells(0).Text
                    DirectCast(Me.Page.Master, MasterPage).ShowMessgae("  ")
                    Response.Redirect("FabricationSheetAdd.aspx?FabricationSheetMstID=" & FabricationSheetMstID & "")

                Case "PDF"
                    For Each Uploadedfiles As String In System.IO.Directory.GetFiles(Server.MapPath("~/TempPDF/"))
                        System.IO.File.Delete(Uploadedfiles)
                    Next
                    Report.Load(Server.MapPath("~/Reports/FabricationSheet.rpt"))
                    Report.Refresh()
                    Dim di As DirectoryInfo = New DirectoryInfo(Server.MapPath("~/TempPDF"))
                    di.Create()
                    Dim StoreReceiptVoucherMstID As Long = dgView.Items(e.Item.ItemIndex).Cells(0).Text

                    Dim FileName As String = "Fabrication Sheet"

...
Public Class SVEntry
    ...
    Dim Dr As DataRow
    Dim tblJVMstID, CargoID As Long
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        CargoID = Request.QueryString("lcargorId")
        If Not Page.IsPostBack Then
            Session("dtDetail") = Nothing
            CARGODATA()
            Dim dtcheck As DataTable = objtblJVMst.CheckCargosv(CargoID)

            If dtcheck.Rows.Count > 0 Then
                EditForSV(dtcheck.Rows(0)("tblJVMstID"))
                ' EditMode()
                '    btnSave.Text = "Update"
            Else
                SetInitialRow()
                '    ' BindBookAccountFirstTime()
                txtVoucherdate.Text = Date.Today
                txtVoucherdate2.Text = Date.Today
                '    btnSave.Text = "Save"
                VoucherNoGenerateOnLoad()
            End If
        End If
        PageHeader("SALES VOUCHER ENTRY FORM")
    End Sub
    Private Sub EditForSV(ByVal ltblJVMstID As Long)
        Dim x As Integer
        Dim dtForEdit As DataTable = objtblJVMst.Edit(ltblJVMstID)
        txtVoucherNo.Text = dtForEdit.Rows(0)("VoucherNo")
        txtVoucherdate.Text = dtForEdit.Rows(0)("VoucherDate")
        txtDescriptionDetail.Text = dtForEdit.Rows(0)("Description")
        '  txtVno.Text = dtForEdit.Rows(0)("VNo")
        ...
        For x = 0 To dtForEdit.Rows.Count - 1
            dr = dt.NewRow()
            dr("tblJvDtlID") = dtForEdit.Rows(x)("tblJvDtlID")
            dr("RowNumber") = x
            dr("AccountCode") = dtForEdit.Rows(x)("AccountCode")
            dr("AccountName") = dtForEdit.Rows(x)("AccountName")
            dr("VoucherType") = dtForEdit.Rows(x)("VoucherType")
            dr("Debit") = dtForEdit.Rows(x)("Debit")
            dr("Credit") = dtForEdit.Rows(x)("Credit")
            dr("TypeID") = dtForEdit.Rows(x)("TypeID")
            dt.Rows.Add(dr)
        Next
        ...
        '---For shipmentdate
        Dim i As Integer
        For i = 0 To dtForEdit.Rows.Count - 1
            ...
            lblRowNo.Text = i + 1
            txtAccountName.Text = dtForEdit.Rows(i)("AccountName")
            txtAccountCode.Text = dtForEdit.Rows(i)("AccountCode")
            cmbType.SelectedValue = dtForEdit.Rows(i)("VoucherType")
            txtDebit.Text = dtForEdit.Rows(i)("Debit")
            txtCredit.Text = dtForEdit.Rows(i)("Credit")
            lblTypeID.Text = dtForEdit.Rows(i)("TypeID")
            cmbType.SelectedValue = lblTypeID.Text
            ...
            '    txtDebit.Text = ""
            'End If
            If dtForEdit.Rows(i)("TypeID") = 1 Then
                txtDebit.Text = Convert.ToDecimal(dtForEdit.Rows(i)("Debit")).ToString("#,000.00")
                txtCredit.Text = ""
            End If
            If dtForEdit.Rows(i)("TypeID") = 2 Then
                txtCredit.Text = Convert.ToDecimal(dtForEdit.Rows(i)("Credit")).ToString("#,000.00")
                txtDebit.Text = ""
            End If
    ...
    End Sub
    Sub CARGODATA()
        Dim dtCARGO As DataTable = objtblJVMst.Cargodata(CargoID)
        If dtCARGO.Rows.Count > 0 Then
            lblInvoiceVal.Text = dtCARGO.Rows(0)("InvoiceValuee")
            txtVno.Text = dtCARGO.Rows(0)("InvoiceNo")
        End If
    End Sub
    Sub VoucherNoGenerateOnLoad()
        Try
            ...
            Dim LastVoucherNo As String = objtblJVMst.GetLastVoucherNoSV(Month, yearP, DayP)
            ...
            Else
                ...
                End If
            End If
            VoucherNo = "SV" & "-" & year & "" & CodeMonth & "" & "" & DayP & "-" & LastCode
                        ...
                        If txtDebit.Text <> "" Then
                            ...
                            Else
                                ...
                                Else
                                    DirectCast(Me.Page.Master, MasterPage).ShowMessgae("")
                                    SaveAllData()
                                    Response.Redirect("~/BusinessProcess/CargoReleaseView.aspx")
                                End If
                            End If

...
Public Class POIssueForAuditor
    ...
    Protected Sub txtSummarySearch_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtSummarySearch.TextChanged
        Try
            Dim dt1 As DataTable = ObjIssue.ViewFabricNewNewPONOViseForIssuedForAuditor(txtSummarySearch.Text)
            Dim dt2 As DataTable = ObjIssue.ViewFabricNewNewForFabricCodeViseViseForAuditor(txtSummarySearch.Text)
            Dim dt3 As DataTable = ObjIssue.ViewFabricNewNewForManulaChalanViseForAuditor(txtSummarySearch.Text)
            Dim dt4 As DataTable = ObjIssue.ViewFabricNewNewForDepartmentViseForAuditor(txtSummarySearch.Text)
            ...
            If dt1.Rows.Count > 0 Then
                ...
                For x = 0 To dgView.Items.Count - 1
                    ...
                    Dim Status As String = dgView.Items(x).Cells(12).Text
                    If Status = 1 Then
    ...
    Protected Sub btnGetDateWise_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnGetDateWise.Click
        Try
            ...
            Else
                ...
                Dim dt As DataTable = objPOMaster.ViewFabricNewNewForUpdateForIssueAuditorNewForDateVise(SDate)
    ...
    Private Sub BindSummaryGridAuth()
        Try
            ...
            For x = 0 To DgAuthorised.Items.Count - 1
                ...
                Dim Status As String = DgAuthorised.Items(x).Cells(12).Text
                If Status = 1 Then
                    ChkAuditorStatus.Checked = True
                Else
                    ChkAuditorStatus.Checked = False
    ...
    Function loadSummaryAuth() As DataTable
        Dim objDataView As DataView
        Dim objDataTable As DataTable
        objDataTable = objPORecvMaster.ViewFabricNewNewForUpdateForIssueAuditor()
        Return objDataTable
    End Function
    Function loadSummary() As DataTable
        Dim objDataView As DataView
        Dim objDataTable As DataTable
        objDataTable = objPORecvMaster.ViewFabricNewNewForUpdateForIssueAuditorNew()
        Return objDataTable
    End Function
    ...
    Protected Sub AuditorStatus(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        For x = 0 To dgView.Items.Count - 1
            ...
            If ChkAuditorStatus.Checked = True Then
                POID = dgView.Items(x).Cells(0).Text
                objJobOrderdatabase.AuditorStatusForRecvIssue(POID)
                objJobOrderdatabase.UpdateAuditorIDForPOIssue(POID, userid)
            Else
                POID = dgView.Items(x).Cells(0).Text
                Dim dt As DataTable = objJobOrderdatabase.CheckAlreadyCheckPOIDForPOrecvIssue(POID)
                If dt.Rows.Count > 0 Then
                Else
                    objJobOrderdatabase.AuditorStatusFalseForPORecvIssue(POID)
                    objJobOrderdatabase.AuditorIDFalseForPOIssue(POID)
                End If
            End If
        Next
        dtSummary = loadSummary()
    ...
    Protected Sub AuditorStatusAuth(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        For x = 0 To DgAuthorised.Items.Count - 1
            ...
            Else
                POID = DgAuthorised.Items(x).Cells(0).Text
                Dim dt As DataTable = objJobOrderdatabase.CheckAlreadyCheckPOIDNewForrecvIssue(POID)
                If dt.Rows.Count > 0 Then
                Else
                    objJobOrderdatabase.AuditorStatusForRecvIssue(POID)
                    objJobOrderdatabase.UpdateAuditorIDForPOIssue(POID, userid)

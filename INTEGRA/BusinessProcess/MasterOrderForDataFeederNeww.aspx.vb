...
Imports System.IO
Public Class MasterOrderForDataFeederNeww
    ...
    Function LoadData() As ICollection
        ...
        End Try
    End Function
    Function LoadDataSearch(ByVal PONO As String) As ICollection
        Try
            Dim objDataView As DataView
            Dim objDataTable As DataTable
            'objDataTable = objPo.GetMasterOrderForDataFeeder()
            If RoleID = 32 Then
                objDataTable = objPo.GetMasterOrderForDataFeederBYManagementBySearch(PONO)
            Else
                objDataTable = objPo.GetMasterOrderForDataFeederBYMerchandiserBySearch(MarchandID, PONO)
            End If
            objDataView = New DataView(objDataTable)
            Return objDataView
        Catch ex As Exception
    ...
    Protected Sub btnAddOrder_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnAddOrder.Click
        Try
            Response.Redirect("POEntryNew.aspx")
        Catch ex As Exception
    ...
    Protected Sub txtPONO_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtPONO.TextChanged
        Try
            ...
            If txtPONO.Text <> "" Then
                objDataview = LoadDataSearch(txtPONO.Text)
                Session("objDataView") = objDataview
                BindGrid()
            Else
                objDataview = LoadData()

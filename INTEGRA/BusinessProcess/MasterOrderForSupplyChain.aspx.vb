...
Imports System.IO
Public Class MasterOrderForSupplyChain
    ...
    Function LoadData() As ICollection
        Try
            ...
            objDataTable = objPo.GetMasterOrderForSupplyChain()
    ...
    Protected Sub dgMasterOrderForSupplyChain_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles dgMasterOrderForSupplyChain.NeedDataSource
        dgMasterOrderForSupplyChain.DataSource = objPo.GetMasterOrderForSupplyChain()
    End Sub
    ...
    Protected Sub cmbAction_SelectedIndexChanged(ByVal sender As Object, ByVal e As Telerik.Web.UI.RadComboBoxSelectedIndexChangedEventArgs) Handles cmbAction.SelectedIndexChanged
        Try
            ...
            ElseIf cmbAction.SelectedValue = 1 Then
                ...
                'End Check
                If StrPODetailID = "()" Then
                    cmbAction.SelectedValue = 0
                    DirectCast(Me.Page.Master, MasterPage).ShowMessgae("No Row Selected")
                Else
                    DirectCast(Me.Page.Master, MasterPage).ShowMessgae(" ")
                    cmbAction.SelectedValue = 0
                    'Response.Write("<script> window.open('LogisticStatusFinalizationPanel.aspx?lPODetailID=" & StrPODetailID & "', 'newwindow', 'left=12, top=30, height=600, width=980, status=no, resizable=no, scrollbars= yes, toolbar=no,location=no, menubar=no,directories=no'); </script>")
                    ScriptManager.RegisterClientScriptBlock(Me.upcmbAction, Me.upcmbAction.GetType(), "New ClientScript", "window.open('LogisticStatusFinalizationPanel.aspx?lPODetailID=" & StrPODetailID & "', 'newwindow', 'left=12, top=30, height=600, width=980, status=no, resizable=no, scrollbars= yes, toolbar=no,location=no, menubar=no,directories=no');", True)
                End If
            ...
            ElseIf cmbAction.SelectedValue = 5 Then
                ...
                Else
                    ...
                    ScriptManager.RegisterClientScriptBlock(Me.upcmbAction, Me.upcmbAction.GetType(), "New ClientScript", "window.open('ProductionTracking.aspx?lPOID=" & CLng(Session("IPOIDSupply")) & "', 'newwindow', 'left=92, top=70, height=550, width=400, status=no, resizable=no, scrollbars= yes, toolbar=no,location=no, menubar=no,directories=no');", True)
                End If
            End If
        Catch ex As Exception

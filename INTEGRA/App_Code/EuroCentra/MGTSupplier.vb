...
Public Class MGTSupplier
 Inherits SQLManager
        Public Sub New()
            m_strTableName = "MGTSupplierReport"
        ...
        Public Property BackLogCleared() As Decimal
            ...
            End Set
        End Property
        Public Function SaveMGTSupplier()
            Try
                MyBase.Save()
        ...
        Public Function TruncateTable()
            Dim str As String
            str = "  truncate table MGTSupplierReport"
            Try
        ...
        Public Function GetMgtSupplierData()
            ...
            End Try
        End Function
        Public Function GetMGTBookedQTY(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetMGTBookedQTYNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Isnull(SUM((POD.Quantity)),0)  as BookedQuantity "
            Str &= " from PurchaseOrder PO "
            Str &= " join PurchaseOrderDetail POD on po.POID=pod.POID "
            ...
            End Try
        End Function
        Public Function GetMGTShippedQTY(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            Str &= " group by po.SupplierID"
            Try
                Return MyBase.GetScaler(Str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetMGTShippedQTYNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Isnull(SUM((cd.quantity)),0)  as ShippedQty "
            Str &= " from Cargodetail cd"
            Str &= " join Cargo c on c.cargoid=cd.cargoid"
            ...
            End Try
        End Function
        Public Function GetMGTShippedQTYOnTime(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            Str &= " and po.status !='Cancel' And PO.SupplierID =" & SupplierID
            Str &= " ) and (DATEDIFF(dd, PO.ShipmentDate , C.ETD))/7 <=0 "

            'Str = " select  Isnull(SUM((CD.Quantity)),0)  as ShippedQuantityOnTime from Cargodetail cd"
            'Str &= " join Cargo c on c.cargoid=cd.cargoid"
            'Str &= " join purchaseorder po on po.poid=cd.popoid "
            'Str &= " where PO.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "' and  po.status !='Close' and po.status !='Cancel' And PO.SupplierID = " & SupplierID
            Try
                Return MyBase.GetScaler(Str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetMGTShippedQTYOnTimeNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Isnull(SUM((CD.Quantity)),0)  as ShippedQuantityOnTime from Cargodetail cd"
            Str &= " join Cargo c on c.cargoid=cd.cargoid"
            Str &= " join purchaseorder po on po.poid=cd.popoid "
            ...
            End Try
        End Function
        Public Function GetMGTBookedForPOs(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetMGTBookedForPOsNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Distinct Count(PO.POID) as TotalBookedPos from PurchaseOrder PO "
            Str &= " where PO.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "'  and po.status !='Cancel' And PO.SupplierID = " & SupplierID
            Try
            ...
            End Try
        End Function
        Public Function GetMGTTotalShippedPos(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetMGTTotalShippedPosNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Count(Distinct PO.POID)  as TotalShippedPOs "
            Str &= " from Cargodetail cd"
            Str &= " join Cargo c on c.cargoid=cd.cargoid"
            ...
            End Try
        End Function
        Public Function GetMGTShippedPOsOnTime(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetMGTShippedPOsOnTimeNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = " select  Count(Distinct PO.POID)  as ShippedPOsOnTime  from Cargodetail cd"
            Str &= " join Cargo c on c.cargoid=cd.cargoid"
            Str &= " join purchaseorder po on po.poid=cd.popoid "
            ...
            End Try
        End Function
        Public Function GetMGTBookedTurOver(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            Str &= " where po.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "' and year(PO.creationDate)>=2012 and po.status !='Cancel' And PO.SupplierID = " & SupplierID
            Try
                Return MyBase.GetDataTable(Str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetMGTBookedTurOverNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = "Select  PO.ExchangeRate , "
            Str &= "  (case when PO.Currency <> 'Dollar' then"
            Str &= "   (Isnull(POD.Quantity,0)*Isnull(POD.Rate,0)) * Isnull(PO.ExchangeRate,0)"
            ...
            End Try
        End Function
        Public Function GetMGTBookedCommNew(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetMGTBookedCommNew2(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = "Select   sum(case when PO.Currency <> 'Dollar' then "
            Str &= " (Convert(Decimal, ( (Isnull((POD.Quantity),0)*Isnull((POD.Rate),0)) "
            Str &= " ),0))*Isnull((PO.ExchangeRate),0)"
            ...
            End Try
        End Function
        Public Function GetMGTShippedTurOver(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select "
            ...
            End Try
        End Function
        Public Function GetMGTShippedTurOverNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select "
            Str &= " (case when C.Currency <> 'Dollar' then "
            Str &= " ("
            ...
            End Try
        End Function
        Public Function GetMGTShippedCommNew(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select Sum("
            Str &= " (case when PO.Currency <> 'Dollar' then "
            Str &= " ("
            ...
            End Try
        End Function
        Public Function GetMGTShippedCommNew2(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select Sum("
            Str &= " (case when PO.Currency <> 'Dollar' then "
            Str &= " ("
            ...
            End Try
        End Function
        Public Function GetMGTClaiPcs(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select  PO.ExchangeRate,SUM(Isnull((C.ClaimPcs),0))  as ClaimPcs from POClaim  C "
            Str &= "  join PurchaseOrder PO on po.POID=c.POID "
            Str &= " where c.creationdate  between '" & Fromdate & "' and '" & ToDate & "' And PO.SupplierID = " & SupplierID
            Try
                Return MyBase.GetScaler(Str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetMGTClaiPcsNew(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long)
            Dim Str As String
            Str = "  select  PO.ExchangeRate,SUM(Isnull((C.ClaimPcs),0))  as ClaimPcs from POClaim  C "
            Str &= "  join PurchaseOrder PO on po.POID=c.POID "
            Str &= " where c.creationdate  between '" & Fromdate & "' and '" & ToDate & "' And PO.SupplierID = " & SupplierID
            ...
            End Try
        End Function
        Public Function GetMGTTotalPos(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            Str = "   select count(PO.POID) as TotalPOs from PurchaseOrder PO"
            Str &= " where po.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "' And PO.SupplierID = " & SupplierID
            Try
                Return MyBase.GetScaler(Str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetMGTTotalPosCheckTimelyShipped(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
        ...
        Public Function GetMGTTotalPOsTimelyShippedSecond(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal POID As Long)
            ...
            End Try
        End Function
        Public Function GetBookedCommission(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            ...
            End Try
        End Function
        Public Function GetBookedEXRate(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long)
            Dim Str As String
            Str = "   select PO.Commission from PurchaseOrder PO"
            Str &= " where po.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "' And PO.SupplierID = " & SupplierID
            Try

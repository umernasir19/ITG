...
Namespace EuroCentra
    Public Class TempSummaryOfWorkLoadDeptWise
        Inherits SQLManager
        Public Sub New()
            m_strTableName = "TempSummaryOfWorkLoadDeptWise"
        ...
        Public Function saveTempSummaryOfWorkLoadDeptWise()
            Try
                MyBase.Save()
        ...
        Public Function TruncateTable()
            Dim str As String = "TRUNCATE TABLE  TempSummaryOfWorkLoadDeptWise"
            Try
                MyBase.ExecuteNonQuery(str)
            Catch ex As Exception
            End Try
        End Function
        Public Function GetVender(ByVal startdate As String, ByVal Enddate As String, ByVal SEASON As String, ByVal CustomerID As Long) As DataTable
            Dim Str As String

            'If supplieridd <> 0 Then
            'If SEASON = "All" Then
            '    Str = "  SELECT distinct EkNumber    "
            ...
            '    Str &= " where po.ShipmentDate between '" & startdate & "' and '" & Enddate & "' "
            ...
            If SEASON = "All" Then
                ...
                Str &= " join Customer V on V.CustomerID=PO.CustomerID"
                Str &= " Join StyleMaster SM ON SM.STYLEID=POD.STYLEID "
                Str &= " where po.ShipmentDate between '" & startdate & "' and '" & Enddate & "' and PO.CustomerID='" & CustomerID & "' order by EkNumber"
            ...
            Else
                ...
                Str &= " Season='" & SEASON & "' and PO.CustomerID='" & CustomerID & "'  order by EkNumber"
                'Else
                '    Str = "  SELECT distinct EkNumber    "
                '    Str &= " FROM PURCHASEORDER po JOIN PurchaseOrderDetail POD ON PO.POID=POD.POID "
                '    Str &= " join Customer V on V.CustomerID=PO.CustomerID"
        ...
        Public Function getdataforWorkLoadSummaryForMonth2(ByVal Month2 As String, ByVal Year As String, ByVal SEASON As String, ByVal BuyingDept As String, ByVal customerid As Long) As DataTable
            ...
            ElseIf SEASON = "All" And BuyingDept <> "All" Then
                ...
                Str &= " JOIN PurchaseOrderDetail POD ON PO.POID=POD.POID"
                Str &= " Join StyleMaster SM ON SM.STYLEID=POD.STYLEID"
                Str &= " where Month(po.ShipmentDate) ='" & Month2 & "' and Year(po.ShipmentDate)='" & Year & "'  "
                Str &= "   and eknumber='" & BuyingDept & "'and PO.CustomerID='" & customerid & "'"
            ElseIf SEASON <> "ALL" And BuyingDept = "All" Then
                Str = "  SELECT isnull(count(distinct PO.Masterpo),0) as Nooforder,isnull(count(distinct POD.STYLEID),0) as Nooflines,"
                Str &= " isnull(sum(POD.Quantity*SM.QtyPack),0) as OrderQtyInPcsM2 ,isnull(sum(POD.Quantity),0) as OrderQtyM2"

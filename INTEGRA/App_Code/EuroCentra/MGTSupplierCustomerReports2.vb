...
Public Class MGTSupplierCustomerReports2
    Inherits SQLManager
    Public Sub New()
    ...
    Public Function GetMarchand() As DataTable
        ...
        End Try
    End Function
    Public Function GetMarchand111() As DataTable
        Dim str As String
        str = " Select Distinct PO.MarchandID, UM.UserName  from PurchaseOrder PO "
        str &= " Join UMUser UM on UM.UserId = PO.MarchandID "
    ...
    Public Function GetMgtSupplierData(ByVal FromDate As Date, ByVal ToDate As Date, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMgtSupplierData1(ByVal FromDate As Date, ByVal ToDate As Date, ByVal CustomerID As Long)
        Dim Str As String
        Str = "  select Distinct  V.venderlibraryid ,V.VenderName from vender V "
        Str &= " join Purchaseorder Po on Po.SupplierID= V.venderlibraryid"
        ...
        End Try
    End Function
    Public Function GetMgtCus(ByVal FromDate As String, ByVal ToDate As String, ByVal CustomerID As String)
        Dim Str As String
        ...
        End Try
    End Function
    Public Function GetMgtCusNew(ByVal FromDate As String, ByVal ToDate As String, ByVal CustomerID As String)
        Dim Str As String
        Str = "  select Distinct C.CustomerID, C.CustomerName From Customer C "
        Str &= " join Purchaseorder Po on Po.CustomerID= C.CustomerID "
        Str &= "   where po.ShipmentDate between '" & FromDate & "' and '" & ToDate & "'   and po.status !='Cancel'  "
        If CustomerID = "0" Or CustomerID = "" Then
    ...
    Public Function GetMgtCustomerData(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMgtCustomerData1(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = "  select Distinct C.CustomerID, C.CustomerName From Customer C "
        Str &= " join Purchaseorder Po on Po.CustomerID= C.CustomerID "
        Str &= "   where po.ShipmentDate between '" & FromDate & "' and '" & ToDate & "'   and po.status !='Cancel' And PO.CustomerID = " & CustomerID
        ...
        End Try
    End Function
    Public Function GetMgtSupMar(ByVal FromDate As String, ByVal ToDate As String, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select Distinct  V.venderlibraryid ,V.VenderName,Po.Marchandid,U.username  from vender V "
        Str &= " join Purchaseorder Po on Po.SupplierID= V.venderlibraryid"
    ...
    Public Function GetMGTBookedQTY(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTBookedQTY1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((POD.Quantity)),0)  as BookedQuantity "
        Str &= " from PurchaseOrder PO "
        Str &= " join PurchaseOrderDetail POD on po.POID=pod.POID "
        ...
        End Try
    End Function
    Public Function GetMGTBookedQTY11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((POD.Quantity)),0)  as BookedQuantity "
        Str &= " from PurchaseOrder PO "
        Str &= " join PurchaseOrderDetail POD on po.POID=pod.POID "
    ...
    Public Function GetMGTShippedQTY(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTShippedQTY1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((cd.quantity)),0)  as ShippedQty "
        Str &= " from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        ...
        End Try
    End Function
    Public Function GetMGTShippedQTY11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((cd.quantity)),0)  as ShippedQty "
        Str &= " from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
    ...
    Public Function GetMGTShippedQTYOnTime(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTShippedQTYOnTime1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((CD.Quantity)),0)  as ShippedQuantityOnTime from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        Str &= " join purchaseorder po on po.poid=cd.popoid "
        ...
        Str &= "   and PO.POID in ( select  Distinct PO.POID from PurchaseOrder PO "
        Str &= " where PO.ShipmentDate  between '" & Fromdate & "' and '" & ToDate & "'   "
        Str &= " and po.status !='Cancel' And PO.SupplierID = '" & SupplierID & "' And PO.CustomerID = '" & CustomerID & "'   "
        Str &= " ) and (DATEDIFF(dd, PO.ShipmentDate , C.ETD))/7 <=0 "
        ...
        End Try
    End Function
    Public Function GetMGTShippedQTYOnTime11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Isnull(SUM((CD.Quantity)),0)  as ShippedQuantityOnTime from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        Str &= " join purchaseorder po on po.poid=cd.popoid "
    ...
    Public Function GetMGTBookedForPOs(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTBookedForPOs1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Distinct Count(PO.POID) as TotalBookedPos from PurchaseOrder PO "
        Str &= " where PO.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "'  and po.status !='Cancel' And PO.SupplierID = " & SupplierID
        Str &= " And PO.CustomerID = '" & CustomerID & "' "
        ...
        End Try
    End Function
    Public Function GetMGTBookedForPOs11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Distinct Count(PO.POID) as TotalBookedPos from PurchaseOrder PO "
        Str &= " where PO.ShipmentDate between '" & Fromdate & "' and '" & ToDate & "'  and po.status !='Cancel' And PO.SupplierID = " & SupplierID
        Str &= " And PO.CustomerID = '" & CustomerID & "' "
    ...
    Public Function GetMGTTotalShippedPos(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTTotalShippedPos1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Count(Distinct PO.POID)  as TotalShippedPOs "
        Str &= " from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        ...
        End Try
    End Function
    Public Function GetMGTTotalShippedPos11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Count(Distinct PO.POID)  as TotalShippedPOs "
        Str &= " from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
    ...
    Public Function GetMGTShippedPOsOnTime(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long)
        ...
        End Try
    End Function
    Public Function GetMGTShippedPOsOnTime1(ByVal Fromdate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long)
        Dim Str As String
        Str = " select  Count(Distinct PO.POID)  as ShippedPOsOnTime  from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        Str &= " join purchaseorder po on po.poid=cd.popoid "
        ...
        End Try
    End Function
    Public Function GetMGTShippedPOsOnTime11(ByVal Fromdate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long)
        Dim Str As String
        Str = " select  Count(Distinct PO.POID)  as ShippedPOsOnTime  from Cargodetail cd"
        Str &= " join Cargo c on c.cargoid=cd.cargoid"
        Str &= " join purchaseorder po on po.poid=cd.popoid "
    ...
    Public Function BookeedTurnOver(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long) As DataTable
        ...
        End Try
    End Function
    Public Function BookeedTurnOver1(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long) As DataTable
        Dim Str As String
        Str = "Select PO.EkNumber, PO.ExchangeRate,  "
        Str &= "  (case when PO.Currency <> 'Dollar' then"
        Str &= "   (Isnull(POD.Quantity,0)*Isnull(POD.Rate,0)) * Isnull(PO.ExchangeRate,0)"
        ...
        End Try
    End Function
    Public Function BookeedTurnOver11(ByVal FromDate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long) As DataTable
        Dim Str As String
        Str = "Select PO.EkNumber, PO.ExchangeRate,  "
        Str &= "  (case when PO.Currency <> 'Dollar' then"
        Str &= "   (Isnull(POD.Quantity,0)*Isnull(POD.Rate,0)) * Isnull(PO.ExchangeRate,0)"
    ...
    Public Function ShippeedTurnOver(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal MarchandID As Long) As DataTable
        ...
        End Try
    End Function
    Public Function ShippeedTurnOver1(ByVal FromDate As Date, ByVal ToDate As Date, ByVal SupplierID As Long, ByVal CustomerID As Long) As DataTable
        Dim str As String
        str = "  select PO.EKNumber, "
        str &= " (case when PO.Currency <> 'Dollar' then "
        str &= " ("
        ...
        End Try
    End Function
    Public Function ShippeedTurnOver11(ByVal FromDate As String, ByVal ToDate As String, ByVal SupplierID As Long, ByVal CustomerID As Long, ByVal Marchandid As Long) As DataTable
        Dim str As String
        str = "  select PO.EKNumber, "
        str &= " (case when PO.Currency <> 'Dollar' then "
        str &= " ("

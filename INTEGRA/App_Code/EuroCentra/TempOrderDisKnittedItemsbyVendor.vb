...
Namespace EuroCentra
    Public Class TempOrderDisKnittedItemsbyVendor
        Inherits SQLManager
        Public Sub New()
            m_strTableName = "TempOrderDisKnittedItemsbyVendor"
        ...
        Public Function TruncateTable()
            Dim str As String = "TRUNCATE TABLE  TempOrderDisKnittedItemsbyVendor"
            Try
        ...
        '        Str &= " FROM PURCHASEORDER po"
        '        Str &= " JOIN PurchaseOrderDetail POD ON PO.POID=POD.POID"
        '        Str &= " where year(po.ShipmentDate)='" & Year & "' AND PO.Supplierid='" & Supplierid & "' and po.customerid='" & customerid & "' and po.EKNumber='" & EKNumber & "'"
        '    End If
        '    Try
        '        Return MyBase.GetDataTable(Str)
        '    Catch ex As Exception
        ...
        Public Function getdataforReport1New(ByVal Year As String, ByVal Supplierid As String, ByVal customerid As String, ByVal EKNumber As String) As DataTable
            ...
            If EKNumber = "All" Then
                ...
                'Str &= " where PO.POID=POD.POID)='Knitwear Apparels'"
        ...
        '        Str &= " FROM PURCHASEORDER po"
        '        Str &= " JOIN PurchaseOrderDetail POD ON PO.POID=POD.POID"
        '        Str &= " where year(po.ShipmentDate)='" & Year & "' and Month(po.ShipmentDate) between '" & Monthone & "' and '" & MonthTwo & "' AND PO.Supplierid='" & Supplierid & "' and po.customerid='" & customerid & "'  "
        '    Else
        '        Str = " SELECT COUNT(DISTINCT po.poiD) AS NoOfOrder, isnull(sum (POD.Quantity),0) as OrderQty"
        '        Str &= " FROM PURCHASEORDER po"
        '        Str &= " JOIN PurchaseOrderDetail POD ON PO.POID=POD.POID"
        '        Str &= " where year(po.ShipmentDate)='" & Year & "' and Month(po.ShipmentDate) between '" & Monthone & "' and '" & MonthTwo & "' AND PO.Supplierid='" & Supplierid & "' and po.customerid='" & customerid & "' and po.EKNumber='" & EKNumber & "'"
        '    End If
        '    Try
        '        Return MyBase.GetDataTable(Str)
        '    Catch ex As Exception
